...
class CustomSelfAttention(nn.Module):
    def __init__(self, d_model, n_head, block_size, dropout=0.1):
        ...
        self.register_buffer('mask',
                             self.generate_square_subsequent_mask(block_size))
        self.n_head = n_head
...
class GPTModel(nn.Module):
    ...
    def configure_optimizers(self, learning_rate, weight_decay, betas):
        ...
        We are separating out all parameters of the model into two buckets: those that will experience weight decay for regularization and those that won't
        (biases, and layernorm/embedding weights). We are then returning the PyTorch optimizer object.
        """
