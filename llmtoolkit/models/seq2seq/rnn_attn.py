...
sys.path.append("../../../")
from llmtoolkit.llms.vanilla.attention import AdditiveAttention
from llmtoolkit.models.seq2seq.rnn_mt import RNNEncoder
...
class RNNeq2SeqModel(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = RNNAttentionDecoder(
            trg_vocab_size, embed_size, hidden_size, num_layers, dropout
        )
...
if __name__ == "__main__":
    ...
    input = torch.LongTensor(
        [[1, 2, 4, 5, 3], [4, 3, 2, 9, 2], [1, 2, 3, 4, 4], [4, 3, 2, 1, 6]]
    )

    target = torch.LongTensor(
        [[1, 3, 4, 5, 3], [4, 3, 2, 9, 2], [1, 2, 3, 4, 4], [4, 3, 2, 1, 6]]
    )
    ...
    decoder = RNNAttentionDecoder(tgt_vocab_size, embed_size, hiddens_size, num_layers)
    enc_outputs = enc_outputs.permute(1, 0, 2)
    dec_outputs, decoder_hidden_state = decoder(target, enc_outputs, enc_state, None)
    print(dec_outputs.shape, decoder_hidden_state.shape)

    print("seq2seq")
    seq2seq = RNNeq2SeqModel(
        src_vocab_size, tgt_vocab_size, embed_size, hiddens_size, num_layers
    )
