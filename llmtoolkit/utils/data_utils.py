...
def minEditDistance(source: str, target: str) -> int:
    ...
    # 初始化编辑距离矩阵
    dist_matrix: List[List[int]] = [[0 for _ in range(m + 1)] for _ in range(n + 1)]
...
def truncate_pad(
    inputs: List[int], max_seq_len: int, padding_token_id: int = 0
) -> List[int]:
...
def get_loader(dataset, batch_size, shuffle=True):
    data_loader = DataLoader(
        dataset, batch_size=batch_size, collate_fn=dataset.collate_fn, shuffle=shuffle
    )
    return data_loader
