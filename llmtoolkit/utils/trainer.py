    ...
    for idx, batch in enumerate(dataloader):
        ...
        if idx % log_interval == 0 and idx > 0:
            ...
            print(
                ...
                "loss {:5.2f}".format(
                    epoch, idx, len(dataloader), elapsed * 1000 / log_interval, cur_loss
                )
            )
            moving_loss = 0
            start_time = time.time()
    ...
    with torch.no_grad():
        for idx, batch in enumerate(dataloader):
            ...
            if idx % log_interval == 0 and idx > 0:
                cur_loss = moving_loss / log_interval
                print(
                    "Val: |{:5d}/{:5d} batches |" "loss {:5.2f} ".format(
                        idx, len(dataloader), cur_loss
                    )
                )
                moving_loss = 0
    return epoch_loss / len(dataloader)
...
def predict_seq2seq(
    ...
    dec_inputs = torch.tensor(tgt_indices, dtype=torch.long).unsqueeze(0).to(device)
    with torch.no_grad():
...
def train_and_evaluate(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        val_loss = evaluate(model, eval_loader, criterion, device, log_interval)
        end_time = time.time()
        ...
        print(f"Valid Loss: {val_loss:.3f} | Valid PPL: {math.exp(val_loss):7.3f}")
