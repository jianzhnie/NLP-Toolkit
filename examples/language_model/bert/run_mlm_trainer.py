...
from nlptoolkit.llms.bert.modeling_bert import BertForPretraing
...
@dataclass
class TrainingArguments_(TrainingArguments):
    # 缓存目录
...
def get_resume_checkpoint(args: argparse.Namespace,
    ...
    if isdir(args.output_dir) and not args.overwrite_output_dir:
        ...
        if last_checkpoint is None and len(os.listdir(args.output_dir)):
            raise ValueError(
                f'Output directory ({args.output_dir}) already exists and is not empty. '
                'Use --overwrite_output_dir to overcome.')
        elif last_checkpoint and args.resume_from_checkpoint is None:
...
def load_hf_datasets(args: argparse.Namespace):
    """
    - Loading Datasets from Hub:

        If args.dataset_name is provided, it downloads and loads the specified dataset from the Hugging Face Model Hub.
        It also handles splitting the dataset into training and validation sets based on the args.validation_split_percentage parameter.

    - Loading Custom Datasets:

        If args.dataset_name is not provided, it assumes custom training and validation files are provided (args.train_file and args.validation_file).
        It loads these files and uses the file extension (either "csv", "json", or "txt") to determine the format.
        If the extension is "txt", it is considered as a dataset of text data.
        If no validation data is provided, the function uses args.validation_split_percentage to split the dataset into training and validation sets.


    Load unsupervised datasets for training and validation.

    Args:
        args (argparse.Namespace): Command line arguments.

    Returns:
        Dict[str, any]: Dictionary containing 'train' and 'validation' datasets.
    """
    ...
    if args.dataset_name is not None:
        # Download and load dataset from the Hugging Face Model Hub.
        ...
        # Handle custom training and validation files.
        data_files = {}
        ...
        if extension == 'txt':
            extension = 'text'
        # Load custom dataset and split into training and validation sets.
...
def main():
    ...
    last_checkpoint = get_resume_checkpoint(args=training_args, logger=logger)

    # Set seed before initializing model.
    set_seed(training_args.seed)

    # Load train and valid datasets
    raw_datasets = load_hf_datasets(args)
